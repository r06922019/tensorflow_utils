...
def test_rgb_to_xyz():
    ...
    linear_rgb_placeholder = tf.placeholder(tf.float32, shape=[batch_size, h, w, c], 
        name="linear_rgb_placeholder")
    tf_xyz_tensor = tf_rgb_to_xyz(linear_rgb_placeholder)

    with tf.Session() as sess:
        for i in range(iterations):
            linear_rgb_images = np.random.random((batch_size, h, w, c)).astype(np.float32)

            ski_xyz = np.asarray([ skimage.color.rgb2xyz(linear_rgb_images[_i]) for _i in range(batch_size) ])
            tf_xyz = sess.run(tf_xyz_tensor, feed_dict={linear_rgb_placeholder:linear_rgb_images})
